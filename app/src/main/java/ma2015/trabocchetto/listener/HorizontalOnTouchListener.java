 ...
 */
public class HorizontalOnTouchListener implements View.OnTouchListener{
    ...
    public HorizontalOnTouchListener(GameActivity gameActivity, Board board) {
    ...
    public boolean onTouch(final View view, final MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (((int) event.getRawX() - initialX) >= movTolerance && (int) event.getRawX() > initialX) {
                    drawableResIdFromAttr= Utils.getResIdFromAttribute(gameActivity, R.attr.arrowRight);
                    imageViewDirection.setImageResource(drawableResIdFromAttr);
                }
                else if((initialX - (int) event.getRawX()) >= movTolerance && (int) event.getRawX() < initialX){
                    drawableResIdFromAttr= Utils.getResIdFromAttribute(gameActivity, R.attr.arrowLeft);
                    imageViewDirection.setImageResource(drawableResIdFromAttr);
                }
                else{
            ...
            case MotionEvent.ACTION_UP: {
                ...
                if ( board.gethBarPosition()[numBar - 1] == OUTER) { //based on the position of the touched bar, I set the correct image if the movement is successful
                    Log.i("FLOW_CONTROL", "Starting in OUTER position...");

                    if (  ((int) event.getRawX() - initialX) >= movTolerance &&  (int) event.getRawX() > initialX   ) {
                        //I'm trying to move the bar to the right (central position)
                        try{
                            board.move('h', numBar, 'i');
                            Log.i("FLOW_CONTROL", "Moving from OUTER to CENTRAL");
                            drawableResIdFromAttr= Utils.getResIdFromAttribute(gameActivity, R.attr.hBarCentral);
                            imageViewBar.setImageResource(drawableResIdFromAttr);
                            gameActivity.prepareForNextMove('h', numBar);
                        }catch (Exception e) {
                        ...
                        if (  ( initialX - (int) event.getRawX()) >= movTolerance &&  (int) event.getRawX() < initialX   ) {
                ...
                } else if ( board.gethBarPosition()[numBar-1] == CENTRAL ) {

                    Log.i("FLOW_CONTROL", "Starting in CENTRAL position...");

                    if (  ((int) event.getRawX() - initialX) >= movTolerance &&  (int) event.getRawX() > initialX   ) {
                        //I'm trying to move the bar to the right (inner position)
                        try{
                            board.move('h', numBar, 'i');
                            Log.i("FLOW_CONTROL", "Moving from CENTRAL to INNER");
                            drawableResIdFromAttr= Utils.getResIdFromAttribute(gameActivity, R.attr.hBarInner);
                            imageViewBar.setImageResource(drawableResIdFromAttr);
                            gameActivity.prepareForNextMove('h', numBar);
                        }catch (Exception e) {
                            GameActivity.toastForbiddenBar.cancel(); //This is done in order to avoid "chain of toasts" accumulating, when a player repeatedly tries to move a forbidden bar
                        ...
                        // I'm trying to move the bar to the left (outer position)
                        try{
                            board.move('h', numBar, 'o');
                            Log.i("FLOW_CONTROL", "Moving from CENTRAL to OUTER");
                            drawableResIdFromAttr= Utils.getResIdFromAttribute(gameActivity, R.attr.hBarOuter);
                            imageViewBar.setImageResource(drawableResIdFromAttr);
                            gameActivity.prepareForNextMove('h', numBar);
                        }catch (Exception e) {
                            GameActivity.toastForbiddenBar.cancel(); //This is done in order to avoid "chain of toasts" accumulating, when a player repeatedly tries to move a forbidden bar
                ...
                } else if ( board.gethBarPosition()[numBar-1] == INNER ) {

                    Log.i("FLOW_CONTROL", "Starting in INNER position...");

                    if (  ( initialX - (int) event.getRawX()) >= movTolerance &&  (int) event.getRawX() < initialX   ) {
                        // I'm trying to move the bar to the left (central position)
                        try{
                            board.move('h', numBar, 'o');
            ...
            case MotionEvent.ACTION_DOWN: {
                //Gets the value of the x coordinate when the user touches-down the screen
